...
def merge_address_ranges(registers: Sequence[AddressRange],
                         allow_holes: bool,
                         max_read_size: int) -> List[AddressRange]:
    ...
    for register in sorted(registers, key=lambda x: (x.address, -x.count)):
        ...
        if cur_rng is not None and \
           cur_rng.address + cur_rng.count > rng.address:
            continue
