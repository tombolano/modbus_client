...
def get_type_format(reg_type: RegisterValueType) -> \
...
class IRegister(AddressRange):
    def __init__(self, name: str, reg_type: RegisterType, address: int,
        ...
        super().__init__(
            address, struct.calcsize(get_type_format(value_type)[0]) // 2)
        self.name = name
    ...
    def get_value_from_read_session(
            self, read_session: ModbusReadSession) -> int:
        ...
        registers_ordered = \
            registers_unordered if not reverse \
            else reversed(registers_unordered)
    ...
    def value_to_modbus_registers(self, value: Union[int, float]) -> List[int]:
        ...
        registers_ordered = \
            registers_unordered if not reverse \
            else list(reversed(registers_unordered))
...
class NumericRegister(IRegister):
    def __init__(self, name: str, reg_type: RegisterType, address: int,
        ...
        super().__init__(name=name, reg_type=reg_type, address=address,
                         value_type=value_type, bits=bits)
        self.scale = scale
        self.unit = unit

    def get_from_read_session(
            self, read_session: ModbusReadSession) -> Union[int, float]:
        num = self.get_value_from_read_session(read_session)
        return num * self.scale

    def value_to_modbus_registers(
            self, value: Union[int, float]) -> List[int]:
...
class Coil(IRegister):
    def __init__(self, name: str, reg_type: RegisterType, number: int) -> None:
        super().__init__(name=name, reg_type=reg_type,
                         address=number // 8 * 8,
                         value_type=RegisterValueType.U16, bits=None)
        self.count = 8
        self.number = number
