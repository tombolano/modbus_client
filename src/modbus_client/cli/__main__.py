...
from modbus_client.client.pymodbus_async_modbus_client import (
...
from modbus_client.device.modbus_device import (
    ModbusDevice,
    create_modbus_coil,
...
def create_device_from_args(args: Args) -> DeviceCreationResult:
    ...
    if device_mode == "tcp":
        ...
        client = PyAsyncModbusRtuClient(
            path=args.path,
            baudrate=args.mode[0],
            stopbits=args.mode[2],
            parity=args.mode[1],
            timeout=3,
        )
    else:
...
async def query_device(
    ...
    modbus_registers_map.update(
        {switch.name: create_modbus_coil(device_config, switch) for switch in switches}
    )
    ...
    def print_registers(
        registers_to_print: Sequence[Union[IDeviceRegister, DeviceSwitch]]
    ) -> None:
...
async def handle_read(
    device_config: DeviceConfig, client: AsyncModbusClient, unit: int, name: str
) -> None:
    ...
    if switch is not None:
        await query_device(
            device_config, client, unit, switches=[switch], show_registers_types=False
        )
        return
...
async def main() -> None:
    ...
    argparser.add_argument(
        "--format", type=str, choices=("raw", "pretty", "json"), default="pretty"
    )

    mode_subparser = argparser.add_subparsers(
        title="standalone device", description="valid subcommands"
    )
    ...
    dev_rtu_p.add_argument(
        "--mode", type=mode_parser, default="9600n1", help="default 9600n1"
    )
    ...
    if args.cmd == "write":
        await handle_write(
            device_config, client, modbus_device, unit, args.name, float(args.value)
        )
